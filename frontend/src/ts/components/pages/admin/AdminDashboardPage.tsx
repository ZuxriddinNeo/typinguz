...
import {
  JSXElement, Show, createSignal, For, createEffect,
} from "solid-js";
import Ape from "../../../ape";
...
function Input(props: {
  ...
  placeholder?: string; type?: string; class?: string;
}): JSXElement {
  return (
    <input
      value={props.value}
      ...
      class={cn(
        "w-full rounded-xl bg-sub-alt p-3 text-sm text-text outline-none ring-1 ring-sub/20 focus:ring-main",
        props.class,
      )}
    />
  );
...
type AdConfig = {
  ...
  creatives: Array<{id: string; name?: string; imageUrl?: string; targetUrl?: string; enabled?: boolean}>;
};
...
export function AdminDashboardPage(): JSXElement {
  ...
  const [pwdState, setPwdState] = createSignal<"idle"|"loading"|"success"|"error">("idle");
  ...
  const [features, setFeatures] = createSignal<Array<{icon:string;title:string;description:string}>>([]);
  const [aboutCards, setAboutCards] = createSignal<Array<{icon:string;title:string;description:string}>>([]);
  ...
  const activityQuery = createQuery(() => ({
    ...
    queryFn: async () => {
      ...
      if (res.status === 200 && res.body.data !== null) return res.body.data.data;
      return [];
    },
    refetchInterval: 60_000,
  }));
  ...
  const searchForm = createForm(() => ({
    ...
    onSubmit: async ({ value }) => {
      try {
        ...
        if (res.status === 200) setSearchResults(res.body.data as Array<{uid:string;name:string;email:string;banned?:boolean}>);
      } catch { showErrorNotification("Qidirishda xatolik"); }
    },
  }));
  ...
  const notificationForm = createForm(() => ({
    ...
    onSubmit: async ({ value }) => {
      ...
      } catch { showErrorNotification("Bildirishnoma yuborishda xatolik"); }
    },
  }));
  ...
  const creativeForm = createForm(() => ({
    ...
    onSubmit: async ({ value }) => {
      ...
      } catch { showErrorNotification("Kreativ qo'shishda xatolik"); }
    },
  }));

  const deleteCreativeMutation = createMutation(() => ({
    mutationFn: async (id: string) => Ape.admin.deleteCreative({ params: { id } }),
    onSuccess: () => {
  ...
  const runAiAnalysis = async () => {
    ...
    } catch { showErrorNotification("AI server xatosi"); }
  ...
  const saveContent = async () => {
    try {
      const body = {
        ...
        footer: { brandName: footerBrand(), tagline: footerTagline(), telegram: footerTelegram() },
      };
      await Ape.admin.updateSiteContent({ body });
      showSuccessNotification("Kontent saqlandi");
    } catch { showErrorNotification("Kontentni saqlashda xatolik"); }
  };

  // --- Save theme ---
  const saveTheme = async () => {
    try {
      await Ape.admin.updateThemeSettings({ body: { accentColor: accentColor(), isDark: isDark() } });
      document.documentElement.style.setProperty("--main-color", accentColor());
      document.documentElement.style.setProperty("--caret-color", accentColor());
      showSuccessNotification("Rang saqlandi");
    } catch { showErrorNotification("Rangni saqlashda xatolik"); }
  };
  ...
  const adToggle = () => {
    ...
    void Ape.admin.updateAdConfig({ body: updated as never })
      .then(() => {
        showSuccessNotification(`Reklama ${updated.enabled ? "yoqildi" : "o'chirilgan"}`);
        void adConfigQuery.refetch();
      })
      .catch(() => showErrorNotification("Reklama sozlamalarini o'zgartirishda xatolik"));
  };
  ...
  const changePassword = async (e: Event) => {
    ...
    if (np !== cf) { setPwdResult("Yangi parollar mos emas"); setPwdState("error"); return; }
    if (np.length < 8) { setPwdResult("Kamida 8 belgi"); setPwdState("error"); return; }
    setPwdState("loading");
    try {
      const token = getStoredToken();
      const res = await fetch(`${envConfig.backendUrl}/auth/admin/change-password`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ currentPassword: cp, newPassword: np }),
      });
      const json = (await res.json()) as { message: string };
      if (res.ok) { setPwdResult("Parol o'zgartirildi"); setPwdState("success"); form.reset(); }
      else { setPwdResult(`Xatolik: ${json.message}`); setPwdState("error"); }
    } catch { setPwdResult("Server xatosi"); setPwdState("error"); }
  };

  // --- Add/remove content items ---
  const addFeature = () => setFeatures([...features(), { icon: "fa-star", title: "", description: "" }]);
  const removeFeature = (i: number) => setFeatures(features().filter((_, idx) => idx !== i));
  const updateFeature = (i: number, field: "icon"|"title"|"description", v: string) => {
    const copy = [...features()] as Array<{icon:string;title:string;description:string}>;
    copy[i] = { ...copy[i], [field]: v } as {icon:string;title:string;description:string};
    setFeatures(copy);
  };
  const addAboutCard = () => setAboutCards([...aboutCards(), { icon: "fa-star", title: "", description: "" }]);
  const removeAboutCard = (i: number) => setAboutCards(aboutCards().filter((_, idx) => idx !== i));
  const updateAboutCard = (i: number, field: "icon"|"title"|"description", v: string) => {
    const copy = [...aboutCards()] as Array<{icon:string;title:string;description:string}>;
    copy[i] = { ...copy[i], [field]: v } as {icon:string;title:string;description:string};
    setAboutCards(copy);
  };
      ...
      <div class="mx-auto flex max-w-6xl flex-col gap-6 px-4 py-8">
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-3">
            <span class="text-xs text-sub">{activeTab()}</span>
            <button
              type="button"
        ...
        {/* Tabs */}
        <div class="flex flex-wrap gap-1 rounded-2xl border border-sub/10 bg-bg/30 p-1">
          <For each={tabs}>
            {(tab) => (
              <button
                type="button"
              ...
              >
                <Fa icon={tab.icon as FaSolidIcon} class="text-xs" />
                {tab.label}
              </button>
            )}
          </For>
        ...
        <Show when={activeTab() === "dashboard"}>
          <div class="grid gap-4 sm:grid-cols-5">
            <div class="rounded-2xl border border-sub/10 bg-bg/50 p-4 text-center">
              <div class="text-2xl font-bold text-text">{analyticsQuery.data?.totalUsers ?? "..."}</div>
              <div class="text-xs text-sub">Foydalanuvchilar</div>
            </div>
            <div class="rounded-2xl border border-sub/10 bg-bg/50 p-4 text-center">
              <div class="text-2xl font-bold text-text">{analyticsQuery.data?.totalTestsCompleted ?? "..."}</div>
              <div class="text-xs text-sub">Testlar</div>
            </div>
            <div class="rounded-2xl border border-sub/10 bg-bg/50 p-4 text-center">
              <div class="text-2xl font-bold text-text">{analyticsQuery.data?.totalTestsStarted ?? "..."}</div>
              <div class="text-xs text-sub">Boshlangan</div>
            </div>
            <div class="rounded-2xl border border-sub/10 bg-bg/50 p-4 text-center">
              <div class="text-2xl font-bold text-text">
                {analyticsQuery.data ? Math.round(analyticsQuery.data.totalTimeTyping / 3600) : "..."}
              </div>
              <div class="text-xs text-sub">Soat</div>
            </div>
            <div class="rounded-2xl border border-sub/10 bg-bg/50 p-4 text-center">
              <div class="text-2xl font-bold text-text">{analyticsQuery.data?.activeUsersLast24h ?? "..."}</div>
                  ...
                  <For each={activityQuery.data}>
                    {(p) => (
                      <div class="flex items-center justify-between rounded-lg bg-sub-alt/30 px-3 py-1.5 text-xs">
                        ...
                        <span class="text-sub">{p.tests} test, {p.users} foydalanuvchi</span>
                      </div>
                    )}
                  </For>
                </div>
        ...
        <Show when={activeTab() === "users"}>
          <div class="grid gap-6 lg:grid-cols-2">
            <Card title="Bloklash">
              <form
                onSubmit={(e) => { e.preventDefault(); void banForm.handleSubmit(); }}
              ...
              >
                ...
                <button type="submit" class="rounded-xl bg-main px-4 py-2 text-sm font-medium text-bg hover:opacity-90">
                  Bloklash/ochish
                </button>
              </form>
              <Show when={banResult()}><p class="mt-2 text-sm text-sub">{banResult()}</p></Show>
            </Card>

            <Card title="Parolni tiklash">
              <form
                onSubmit={(e) => { e.preventDefault(); void emailForm.handleSubmit(); }}
                class="flex flex-col gap-3"
              >
            ...
            <Card title="Qidirish">
              <form
                onSubmit={(e) => { e.preventDefault(); void searchForm.handleSubmit(); }}
                class="flex flex-col gap-3"
              >
                  ...
                  <For each={searchResults()}>
                    {(u) => (
                      <div class="rounded-lg bg-sub-alt/50 p-2 text-xs text-text">
                        ...
                        <span class="ml-2 text-sub">({u.uid.slice(0, 8)}...)</span>
                        <Show when={u.banned}><span class="ml-2 text-error">[bloklangan]</span></Show>
                      </div>
                    )}
                  </For>
                </div>
            ...
            <Card title="Bildirishnoma yuborish">
              <form
                onSubmit={(e) => { e.preventDefault(); void notificationForm.handleSubmit(); }}
                class="flex flex-col gap-3"
              >
                <notificationForm.Field name="uid">
                  {(f) => (
                    <input value={f().state.value} onInput={(e) => f().handleChange(e.currentTarget.value)}
                      placeholder="Foydalanuvchi UID"
                      class="w-full rounded-xl bg-sub-alt p-3 text-sm text-text outline-none ring-1 ring-sub/20 focus:ring-main" />
                  )}
                ...
                <notificationForm.Field name="body">
                  {(f) => (
                    <textarea value={f().state.value} onInput={(e) => f().handleChange(e.currentTarget.value)}
                      placeholder="Matn" rows={3}
                      class="w-full resize-none rounded-xl bg-sub-alt p-3 text-sm text-text outline-none ring-1 ring-sub/20 focus:ring-main">
                    </textarea>
                  )}
                </notificationForm.Field>
                <button type="submit" class="rounded-xl bg-main px-4 py-2 text-sm font-medium text-bg hover:opacity-90">
              ...
              <Show when={notificationResult()}><p class="mt-2 text-sm text-sub">{notificationResult()}</p></Show>
            </Card>
          </div>
        </Show>
        ...
        <Show when={activeTab() === "content"}>
          <Card title="Bosh sahifa — Hero">
            <div class="flex flex-col gap-3">
              <Input value={heroTitle()} onInput={setHeroTitle} placeholder="Sarlavha (TypeUZ)" />
              <Input value={heroSubtitle()} onInput={setHeroSubtitle} placeholder="Kichik sarlavha" />
              <Input value={heroDesc()} onInput={setHeroDesc} placeholder="Ta'rif" />
            </div>
          </Card>
                ...
                {(f, i) => (
                  <div class="flex items-start gap-2 rounded-lg bg-sub-alt/30 p-3">
                    <div class="flex flex-1 flex-col gap-2">
                      <Input value={f.icon} onInput={(v) => updateFeature(i(), "icon", v)} placeholder="Icon (fa-star)" />
                      <Input value={f.title} onInput={(v) => updateFeature(i(), "title", v)} placeholder="Sarlavha" />
                      <Input value={f.description} onInput={(v) => updateFeature(i(), "description", v)} placeholder="Ta'rif" />
                    </div>
            ...
            <div class="flex flex-col gap-4">
              <For each={aboutCards()}>
                {(c, i) => (
                  <div class="flex items-start gap-2 rounded-lg bg-sub-alt/30 p-3">
                    <div class="flex flex-1 flex-col gap-2">
                      <Input value={c.icon} onInput={(v) => updateAboutCard(i(), "icon", v)} placeholder="Icon (fa-language)" />
                      <Input value={c.title} onInput={(v) => updateAboutCard(i(), "title", v)} placeholder="Sarlavha" />
                      <Input value={c.description} onInput={(v) => updateAboutCard(i(), "description", v)} placeholder="Ta'rif" />
                    </div>
                    <button type="button" onClick={() => removeAboutCard(i())}
                      class="mt-1 rounded-lg bg-error/20 p-2 text-error hover:bg-error hover:text-bg">
                      <Fa icon="fa-times" />
                    </button>
                  </div>
          ...
          <Card title="Footer">
            <div class="flex flex-col gap-3">
              <Input value={footerBrand()} onInput={setFooterBrand} placeholder="Brand nomi" />
              <Input value={footerTagline()} onInput={setFooterTagline} placeholder="Shior" />
              <Input value={footerTelegram()} onInput={setFooterTelegram} placeholder="Telegram link" />
            </div>
          </Card>
          ...
          <div class="flex justify-end">
            <button type="button" onClick={saveContent}
              ...
              <Fa icon="fa-save" class="mr-2" />Kontentni saqlash
            </button>
          </div>
        </Show>
            ...
            <div class="flex flex-col gap-4">
              <div class="flex items-center gap-4">
                ...
                <div class="flex flex-col">
                  ...
                  <span class="text-xs text-sub">Asosiy to'q sariq rang (#ff5a1f)</span>
                </div>
              </div>
              <div class="flex items-center gap-3">
              ...
              <button type="button" onClick={saveTheme}
                ...
                <Fa icon="fa-save" class="mr-2" />Rangni saqlash
              </button>
            </div>
          </Card>
        </Show>
        ...
        <Show when={activeTab() === "ads"}>
          <div class="grid gap-6 lg:grid-cols-2">
              ...
              >
                <div class="space-y-3 text-sm">
                  <div class="flex items-center justify-between">
                    ...
                    <button type="button" onClick={adToggle}
                      class={`rounded-lg px-3 py-1 text-xs font-medium transition-colors ${
                        (adConfigQuery.data as AdConfig | undefined)?.enabled
                          ? "bg-green-600 text-white" : "bg-sub-alt text-sub"
                      }`}>
                      {(adConfigQuery.data as AdConfig | undefined)?.enabled ? "Yoqilgan" : "O'chirilgan"}
                    </button>
                  </div>
                  <div class="flex items-center justify-between">
                    <span class="text-text">Master kalit</span>
                    <span class={`rounded-lg px-3 py-1 text-xs font-medium ${
                      (adConfigQuery.data as AdConfig | undefined)?.masterToggle
                        ? "bg-green-600 text-white" : "bg-sub-alt text-sub"
                    }`}>
                      {(adConfigQuery.data as AdConfig | undefined)?.masterToggle ? "Yoqilgan" : "O'chirilgan"}
                    </span>
                  </div>
                  <p class="text-xs text-sub">
                    Slotlar: {(adConfigQuery.data as AdConfig | undefined)?.slots.length ?? 0} |
                    Kreativlar: {(adConfigQuery.data as AdConfig | undefined)?.creatives.length ?? 0}
                  </p>
                  <Show when={((adConfigQuery.data as AdConfig | undefined)?.creatives.length ?? 0) > 0}>
                    <div class="mt-2 max-h-32 space-y-1 overflow-y-auto">
                      <For each={(adConfigQuery.data as AdConfig | undefined)?.creatives ?? []}>
                        {(cr) => (
                          <div class="flex items-center justify-between rounded-lg bg-sub-alt/30 px-3 py-1.5 text-xs">
                            <span class="truncate text-text">{(cr.imageUrl ?? cr.id).slice(0, 40)}...</span>
                            <button type="button" onClick={() => deleteCreativeMutation.mutate(cr.id)}
            ...
            <Card title="Kreativ qo'shish">
              <form onSubmit={(e) => { e.preventDefault(); void creativeForm.handleSubmit(); }} class="flex flex-col gap-3">
        ...
        <Show when={activeTab() === "ai"}>
          <Card title="AI Weekly Analysis — Test">
            <p class="mb-4 text-sm text-sub">
              Bu tugma haqiqiy foydalanuvchi ma'lumotlaridan so'nggi 7 kunlik tahlilni chaqiradi.
              Cheklov yo'q — istalgan vaqtda bosishingiz mumkin.
            </p>
            <button type="button" onClick={runAiAnalysis}
              ...
              {aiLoading() ? "Yuklanmoqda..." : <><Fa icon="fa-brain" class="mr-2" />Tahlilni ishga tushirish</>}
            </button>
              ...
              <div class="mt-6 rounded-2xl border border-sub/10 bg-bg/50 p-6">
                ...
                <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
                  <div>
                    ...
                    <div class="text-2xl font-bold text-text">
                      {(aiResult() as unknown as AiAnalysis | undefined)?.avgWpm ?? "—"}
                    </div>
                  </div>
                  <div>
                    <span class="text-xs text-sub">Aniqlik</span>
                    <div class="text-2xl font-bold text-text">
                      {(aiResult() as unknown as AiAnalysis | undefined)?.avgAccuracy ?? "—"}%
                    </div>
                  </div>
                  <div>
                    <span class="text-xs text-sub">Testlar</span>
                    <div class="text-2xl font-bold text-text">
                      {(aiResult() as unknown as AiAnalysis | undefined)?.totalTests ?? "—"}
                    </div>
                  </div>
                  <div>
                    <span class="text-xs text-sub">Eng yaxshi WPM</span>
                    <div class="text-2xl font-bold text-text">
                      {(aiResult() as unknown as AiAnalysis | undefined)?.bestWpm ?? "—"}
                    </div>
                  </div>
                </div>
                <div class="mt-3 flex items-center gap-2 text-xs">
                  ...
                  <span class={
                    (aiResult() as unknown as AiAnalysis | undefined)?.trend === "improving" ? "text-green-400" :
                    (aiResult() as unknown as AiAnalysis | undefined)?.trend === "declining" ? "text-red-400" : "text-sub"
                  }>
                    {(aiResult() as unknown as AiAnalysis | undefined)?.trend ?? "—"}
                  </span>
                </div>
                <Show when={(aiResult() as unknown as AiAnalysis | undefined)?.recommendation}>
                  <div class="mt-3 rounded-lg bg-sub-alt/30 p-3 text-xs text-sub">
                    <span class="font-semibold text-text">Tavsiya: </span>
                    {(aiResult() as unknown as AiAnalysis | undefined)?.recommendation}
                  </div>
                </Show>
              </div>
            </Show>
          ...
          <div class="grid gap-6 lg:grid-cols-2">
            <Card title="Parolni o'zgartirish">
              <form onSubmit={changePassword} class="flex flex-col gap-3">
                <input name="currentPassword" type="password" placeholder="Joriy parol"
                  autocomplete="current-password" required minLength={1}
                  class="w-full rounded-xl bg-sub-alt p-3 text-sm text-text outline-none ring-1 ring-sub/20 focus:ring-main" />
                <input name="newPassword" type="password" placeholder="Yangi parol (8+ belgi)"
                  autocomplete="new-password" required minLength={8}
                  class="w-full rounded-xl bg-sub-alt p-3 text-sm text-text outline-none ring-1 ring-sub/20 focus:ring-main" />
                <input name="confirmPassword" type="password" placeholder="Yangi parolni takrorlang"
                  autocomplete="new-password" required minLength={8}
                  class="w-full rounded-xl bg-sub-alt p-3 text-sm text-text outline-none ring-1 ring-sub/20 focus:ring-main" />
                <button type="submit" disabled={pwdState() === "loading"}
                  ...
                  {pwdState() === "loading" ? "Yuborilmoqda..." : "Parolni o'zgartirish"}
                </button>
                <Show when={pwdResult()}>
                  <p class={`mt-1 text-sm ${pwdState() === "success" ? "text-green-500" : "text-error"}`}>
                    {pwdResult()}
                  </p>
                </Show>
              </form>
...
function ChartBars(props: {
  ...
  const maxVal = () => Math.max(1, ...props.data.map((d) => (d[props.valueKey] as number) ?? 0));
          ...
          {(d) => {
            ...
            return (
              <div class="flex flex-1 flex-col items-center gap-1">
                ...
                ></div>
                <span class="text-[8px] text-sub/60 truncate w-full text-center">{label}</span>
              </div>
            );
          }}
        </For>
